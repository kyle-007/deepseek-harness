...
- `workspace-write` — allows a mutation when the target canonicalizes under a writable root: the workspace root plus the platform temp areas (`/tmp`, `os.tmpdir()`), the SAME set the Seatbelt profile grants, derived from the one [`writableRoots`](../../sandbox/README.md) function so the fs fence and the bash runner cannot drift. Canonical spellings use a lexical fast path; an identity-based ancestor fallback recognizes alias-equivalent roots such as Windows long names and 8.3 names without treating unrelated prefixes as contained. A target the canonical check refuses is allowed when the declared path — the spelling the caller asked for, with `.` and `..` already collapsed — is lexically under a writable root, so a directory symlinked into the workspace stays writable. The target is re-canonicalized immediately before delegating, so an ancestor symlink swapped since the tool resolved it is caught.
...
- **The declared-path allowance follows symlinks out of the workspace** — a target whose declared path is lexically inside a writable root is allowed even when it canonicalizes elsewhere, so a symlink placed inside the workspace grants writes wherever it points. The fs fence and the bash runner therefore no longer confine to the same set.
