...
- **符号链接进工作区的目录不可写**：包含关系依据规范化路径判定，因此工作区内指向别处的链接会被拒绝，即使是开发者刻意创建的。改用声明路径判定会让模型创建的任意符号链接写到任何位置；要让这类目录可写，必须把它的规范化目标授权为可写根，而这属于 `writableRoots` 的决策，以保证 fs 围栏与 bash runner 约束到同一集合。
