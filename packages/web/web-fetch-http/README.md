...
- **Only statelessly decoded charsets are accepted** — `iso-2022-jp` (in-band escape sequences switch character sets) and `x-user-defined` (raw bytes passed into a private-use range) throw `WEB_UNSUPPORTED_CONTENT_TYPE`, so a server-declared charset cannot change what the response bytes mean. Every other label `TextDecoder` recognizes still decodes, including `shift_jis`, `euc-jp`, `gbk`, `gb18030`, `big5`, and `euc-kr`.
