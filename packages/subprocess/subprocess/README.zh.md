...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称与 `DSH_*` 名称都会被丢弃，显式 `env` 在清除之后合并。`FORWARDED_CREDENTIAL_ENV` 是这条丢弃规则的封闭豁免——那些若被清除就会让 `gh`、`npm`、`aws`、`docker` 停在登录提示上的开发者 CLI 凭据——并且受变更管控：每个名称的用途、所授予的访问范围与评审引用都记录在[子进程子系统页面](../../../docs/subsystems/subprocess.zh.md)上，由 `pnpm run verify-forwarded-credential-env` 保证其与集合一致。本地的普通 spawn 与终端 spawn 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
