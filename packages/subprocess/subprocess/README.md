...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped and `DSH_*` names are dropped, and explicit `env` merges after the scrub. `FORWARDED_CREDENTIAL_ENV` is the closed exemption to that drop — developer-CLI credentials that would otherwise leave `gh`, `npm`, `aws`, and `docker` waiting at a login prompt — and it is change-controlled: each name's use case, granted reach, and review reference are recorded on the [subprocess subsystem page](../../../docs/subsystems/subprocess.md), which `pnpm run verify-forwarded-credential-env` holds to the set. The local ordinary and terminal spawns both apply it; SDK-managed transports that own their spawn may import it directly.
